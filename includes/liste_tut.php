    ...
    <section class="container text-center bg-light py-5">
            <table class="table" id="test">
                ...
                <tbody>
                    ...
                    $sql = "SELECT * FROM utilisateur WHERE tuteur = '1';" ; //selectionner la base de donnée UTILISATEUR
                    $result = $db->query($sql);
                    ...
                    <td>' .$row["email"].'</td>
                    <td>
                        <form method="POST" enctype="multipart/form-data" action="../pages/mod_tut.php?id='.$row['id_user'].'">
                            <input class="btn-sm btn-primary mb-4" type="submit" value="Modifier" name="" >
                        </form>
                        <button type="button" class="btn-sm btn-secondary modalBtn" onclick= supTut("'.$row['id_user'].'") data-toggle="modal" data-target="#supmodal">
                        Supprimer
                        </button>
                    </td>
                    </td>
...
<?php include '../modal/modal_tut.php'; ?>
</body>
</html>
